...
// Mirrored from main.js so we can build the allowed-Origin allowlist for the
// WS verifyClient hook. Keep in sync if main.js's HTTP_PORT changes.
const HTTP_PORT_FOR_ORIGIN = 49080;
...
//   In some match modes (notably bot exhibitions and certain casual
//   playlists) RL's Stats API doesn't fire GoalScored / GoalReplayStart /
//   GoalReplayEnd / CountdownBegin, and StatfeedEvent never includes
//   demolitions. UpdateState DOES carry per-team Score and per-player
//   Goals / Demos fields though, so we watch deltas across consecutive
//   UpdateState frames and synthesize the missing events. Overlay + OBS
//   auto-switch + demo counter all consume the synthesized events as if
//   they came from RL natively. Tagged _synthetic so a future consumer
//   can tell them apart if needed.
// =============================================================================
function createSyntheticDeriver() {
  let last = null; // { guid, teamScores: [...], playerGoals: {}, playerDemos: {} }
  return function derive(env) {
    if (!env || env.event !== "UpdateState" || !env.data) return [];
    const data = env.data;
    const guid = data.MatchGuid;
    const teams = (data.Game && data.Game.Teams) || [];
    const players = data.Players || [];
    if (!last || last.guid !== guid) {
      // New match - seed baseline, don't fire on the first frame.
      last = { guid, teamScores: [], playerGoals: {}, playerDemos: {} };
      for (let t = 0; t < teams.length; t++) last.teamScores[t] = teams[t]?.Score ?? 0;
      for (const p of players) {
        last.playerGoals[p.Name] = p.Goals ?? 0;
        last.playerDemos[p.Name] = p.Demos ?? 0;
      }
      return [];
    }
    const out = [];
    // Goal detection: team Score went up since last frame. Find the player
    // on that team whose personal Goals also incremented = the scorer.
    // Own goals (no player Goals delta on scoring team) silently skip.
    for (let t = 0; t < teams.length; t++) {
      const cur = teams[t]?.Score ?? 0;
      const prev = last.teamScores[t] ?? 0;
      if (cur > prev) {
        const scorer = players.find(
          (p) => p.TeamNum === t && (p.Goals ?? 0) > (last.playerGoals[p.Name] ?? 0)
        );
        if (scorer) {
          out.push({
            event: "GoalScored",
            data: {
              MatchGuid: guid,
              Scorer: { Name: scorer.Name, TeamNum: t },
              _synthetic: true,
            },
          });
        }
      }
      last.teamScores[t] = cur;
    }
    // Demo detection: per-player Demos counter went up.
    for (const p of players) {
      const cur = p.Demos ?? 0;
      const prev = last.playerDemos[p.Name] ?? 0;
      if (cur > prev) {
        out.push({
          event: "StatfeedEvent",
          data: {
            MatchGuid: guid,
            Type: "Demolition",
            Attacker: { Name: p.Name, TeamNum: p.TeamNum },
            _synthetic: true,
          },
        });
      }
      last.playerDemos[p.Name] = cur;
    }
    // Keep Goals tracking in sync regardless (might have ticked without a
    // team Score change in weird modes).
    for (const p of players) last.playerGoals[p.Name] = p.Goals ?? 0;
    return out;
  };
}
...
function startBridge(opts = {}) {
  ...
  // / ::, which exposed the control bus to anyone on the producer's WiFi.)
  // Also reject WS handshakes whose Origin header isn't our own control
  // panel page; this blocks browser-tab CSWSH where a random website opens
  // ws://localhost:49777 from JS and starts sending settings.
  const BIND_HOST = "127.0.0.1";
  const ALLOWED_ORIGINS = new Set([
    `http://localhost:${HTTP_PORT_FOR_ORIGIN}`,
    `http://127.0.0.1:${HTTP_PORT_FOR_ORIGIN}`,
  ]);
  function verifyClient({ origin }) {
    // OBS Browser Source and the overlay tab send no Origin header at all;
    // accept those. Anything that DOES send an Origin must match our app.
    return origin === undefined || origin === "" || ALLOWED_ORIGINS.has(origin);
  }

  // --- game feed: broadcast RL frames to overlay clients ---
  const gameWss = new WebSocketServer({ port: GAME_WS_PORT, host: BIND_HOST, verifyClient });
...
function connectRocketLeague(broadcastGame) {
  ...
  sock.on("data", (chunk) => {
    for (const raw of framer.push(chunk.toString())) {
      ...
      if (!env) continue;
      broadcastGame(env);
      for (const syn of derive(env)) broadcastGame(syn);
    }
  });
