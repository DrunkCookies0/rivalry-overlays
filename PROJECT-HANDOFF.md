...
Background: leagues used to build overlays on BakkesMod (a Rocket League mod) plus its SOS
plugin. Psyonix removed that path. Rocket League now ships an OFFICIAL, sanctioned Stats API
("MatchStatsExporter_TA"), and this project is built on that. No game injection, no mods,
nothing that can get a player banned. The popular paid tool in this space is BARL; this is
our own equivalent, fully under our control.
...
This local-first design means the overlay works offline and is fully self-contained. (BARL,
by contrast, loads its UI from a remote website; see Roadmap for the tradeoff.)
...
- Optional hybrid hosting: serve the overlay/control from a website (like BARL) so UI tweaks
