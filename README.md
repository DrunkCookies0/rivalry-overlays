...
Windows installer. It does the same thing BARL does, but it is yours: a control
panel, a background bridge, and an OBS overlay you can restyle however you like.
...
This is Psyonix's sanctioned replacement for the old BakkesMod + SOS route that
leagues used to rely on. No game injection, no third party mod.
...
3. **The app bridges TCP to WebSocket.** Browsers cannot open raw TCP sockets, so the
   ...
   skip, and it is exactly what BARL does internally.
...
("More info" -> "Run anyway"). This is normal for indie tools (BARL is the same). To
remove the warning, buy a code-signing certificate and add `win.certificateFile` /
...
inject into the game, so it is the supported path. The overlay code here is original
RIVALRY work, not copied from BARL. Keep your branding and assets your own.
